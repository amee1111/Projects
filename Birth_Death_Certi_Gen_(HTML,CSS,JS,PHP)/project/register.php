<?php
    if ($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if ($conn) {
            $ins = "INSERT INTO register(fname, lname, mobile, address, password) VALUES('".$_POST['fn']."', '".$_POST['ln']."', '".$_POST['mobile']."', '".$_POST['add']."', '".$_POST['pw']."')";
            if(mysqli_query($conn,$ins)){
                echo "<script>alert('Registration successfull!!');window.location.href='user.php'</script>";
            }
            else {
                echo "<script>alert('Registration error!!');window.location.href='register.php'</script>";   
            }
        }else{
            echo "<script>alert('DB Connection Error');window.location.href='user.php'</script>";
        }
...
<body>
	...
	<div class="form-box2" style="border-radius: 25px;">
		<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
			<h3 class="h1">User Registration Form</h3><br><br>
            ...
            <a href="user.php" class="link4">Do you have an account? SIGN IN</a>
